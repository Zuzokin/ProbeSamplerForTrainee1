...
namespace ProbeSampler.WPF
{
    public class LocalFileStorage : IStorageService
    {
        ...
        public IEnumerable<T> Load<T>(string pathToFolder) where T : ISavableEntity
        {
        ...
        public async Task<IEnumerable<T>> LoadAsync<T>(string pathToFolder) where T : ISavableEntity
        {
            ThrowIfDirectoryNotExists(pathToFolder);
        ...
        public T? Load<T>(string nameToLoad, string pathToFolder) where T : ISavableEntity
        {
            ThrowIfDirectoryNotExists(pathToFolder);

            var file = Directory.GetFiles(pathToFolder).FirstOrDefault(s => Path.GetFileName(s) == nameToLoad);
            if (file == null)
        ...
        public async Task<T?> LoadAsync<T>(string nameToLoad, string pathToFolder) where T : ISavableEntity
        {
            ThrowIfDirectoryNotExists(pathToFolder);
        ...
        public void Remove(string nameToRemove, string pathToFolder)
        {
            ...
            string? file = Directory.GetFiles(pathToFolder)
                .FirstOrDefault(s => Path.GetFileName(s) == nameToRemove);
