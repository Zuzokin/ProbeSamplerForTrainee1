...
namespace ProbeSampler.Core.Services.Opc
{
    public class OpcUaClientHelper : IOpcUaClient
    {
        ...
        /// <summary>
        /// Создание сертификата(не работает)
        /// </summary>
        ...
        public async Task<string> ReadValues(int numberOfPos)
        {
            ...
            posValues = await Client?.ReadNodesAsync<UInt16>(posStr.ToArray());
            for (int i = 0; i < posStr.Count(); i += 2)
            {
                resultString += $"{posStr[i]} -- {posValues[i]}; {posStr[i+1]} -- {posValues[i+1]};{Environment.NewLine}";
            }
            otherValues = await Client?.ReadNodesAsync<int>(otherStr.ToArray());
            for (int i = 0; i < otherStr.Count(); i++)
            {
                resultString += $"{otherStr[i]} -- {otherValues[i]};{Environment.NewLine}";
            }
            return resultString;
        }
        ...
        public async Task<string> ReadAllValues(byte MaxDots = 14)
        {
            ...
            for (int i = 1; i <= MaxDots; i++)
            {
                opcServerValues.Add($"pos mast {i} -- {await ReadValue<int>($"ns=1;s=SIEMENSPLC.siemensplc.pos mast {i}")}");
                opcServerValues.Add($"pos kopf {i} -- {await ReadValue<int>($"ns=1;s=SIEMENSPLC.siemensplc.pos kopf {i}")}");                
            }
            opcServerValues.Add($"anzahl_positiionen -- {await ReadValue<int>($"ns=1;s=SIEMENSPLC.siemensplc.anzahl_positiionen")}");
            opcServerValues.Add($"status_sps -- {await ReadValue<int>($"ns=1;s=SIEMENSPLC.siemensplc.status_sps")}");
            opcServerValues.Add($"aktionen -- {await ReadValue<int>($"ns=1;s=SIEMENSPLC.siemensplc.aktionen")}");

            statusMessage += string.Join(Environment.NewLine, opcServerValues);
