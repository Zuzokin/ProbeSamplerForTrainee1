...
namespace ProbeSampler.Presentation
{
    ...
    public class ConfigManagerViewModel : RoutableViewModelBase, INestedMessageSender
    {
        ...
        protected override void SetupCommands()
        {
            ...
            LogInCommand = ReactiveCommand.Create(() => LogIn(Password));
        ...
        private Unit EditConfig(Guid id)
        {
            if (AuthPasswordStatus == AuthorizationStatus.Success)
            {
                ...
                //MessageBox.Show("Для редактирования необходимо авторизоваться", "Необходима авторизация", MessageBoxButton.OK, MessageBoxImage.Warning);
        ...
        {
            //todo перенести message box в code behind ?
            if (AuthPasswordStatus == AuthorizationStatus.Success)
            {
                ...
                //MessageBox.Show("Для удаления необходимо авторизоваться", "Необходима авторизация", MessageBoxButton.OK, MessageBoxImage.Warning);
        ...
        private void CreateConfig()
        {
            if (AuthPasswordStatus == AuthorizationStatus.Success)
            {
                ...
                //MessageBox.Show("Для создания необходимо авторизоваться", "Необходима авторизация", MessageBoxButton.OK, MessageBoxImage.Warning);
        }
        ...
        private async void LogIn(string password)
        {
            ...
            //        if (settings.AdminPassword == null)
            //        {
            //            //todo добавить уведомление, что пароль не задан
            //        }
            //        if (!PasswordHasher.VerifyIdentityV3Hash(password, settings.AdminPassword))
            //        {
            //            AuthPasswordStatus = AuthorizationStatus.WrongPassword;
            //            this.Log().Info($"Failed attempt to login, wrong password");
            //            this.SendMessageToBus("Неверный пароль");
            //            return;
            //        };
            //        AuthPasswordStatus = AuthorizationStatus.Success;
            //        this.Log().Info($"Authorization has been made");
            //        this.SendMessageToBus("Вы успешно авторизовались");
            //    }
            //}
            //catch
            //{
        ...
        private void LogOut()
        {
            ...
            this.SendMessageToBus("Вы разлогинились");
        }
        ...
        //{
        //    ChangingPasswordStatus = (ChangingPasswordStatus == ChangePasswordMenuStatus.Closed) ? ChangePasswordMenuStatus.WaitForPassword : ChangePasswordMenuStatus.Closed;
        //}
    }
}
