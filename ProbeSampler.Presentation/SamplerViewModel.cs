...
namespace ProbeSampler.Presentation
{
    public class SamplerViewModel : ViewModelBase, ITabContent, IDisposable
    {
        ...
        private IEnumerable<BoundingBox>? currentBoxes = null;

        [Reactive] public CrossbarType CrossbarType { get; set; }
        [Reactive] public GridEditMode DeletedCellsMode { get; set; }
        ...
        [Reactive] public bool DeleteCellsSwitch { get; set; } = false;
        ...
        private Mat CameraVM_ImageProcessingPipeline(Mat input)
        {
            ...
            if (yolov8Processor != null && DetectionType == DetectionType.Yolov8)
            {
                ...
                currentBoxes = (yolov8Processor as IProvideBoxes)?.GetDetections();
                // foreach (var box in boxes)
                // {
                //    var test = box.Width;
                // }

                CellsNumberToSelect = CellsHelper.GetCellNumberToSelect(currentBoxes);
                //CellsToSelectMultiplier = boxes.Count() > 1 || boxes.Any(box => box.Width > BigTruckWidth) ? CellsToSelectBaseMultiplier : 1;
                CellsHelper.UpdateCellsState(Cells, currentBoxes);
        ...
        protected override void SetupSubscriptions(CompositeDisposable d)
        {
            ...
            this.WhenAnyValue(x => x.CrossbarSwitch)
                ...
                .DisposeWith(d);
            this.WhenAnyValue(x => x.DeleteCellsSwitch)
                .Subscribe(switchValue =>
                {
                    DeletedCellsMode = switchValue ? GridEditMode.Cells : GridEditMode.Crossbars;
                })
                .DisposeWith(d);
        ...
        protected override void SetupCommands()
        {
            ...
            SelectRandomCellsCommand = ReactiveCommand.CreateFromTask(async () => await Task.Run(() => CellsHelper.SelectRandomCells(Cells, currentBoxes)), canInteractSells);
        ...
        /// </summary>
        /// <param name="SelectedCells">лист выбранных клеток</param>
        private async Task OpcTakeProbe(List<Cell> SelectedCells)
        {
            this.Log().Info($"Starts sets values to Opc, current boxes: {currentBoxes}");
            try
            {
                if (SelectedCells is null || SelectedCells.Count == 0)
                {
                    this.SendMessageToBus("Не выбрано ни одной клетки");
                    this.Log().Warn("Doesn't select any cells");
                    return;
                }
                ...
                {                    
                    //Если включен ручной режим, клетки не убираем, даже если не распознался кузов
                    ...
                    await opcUaClient.ResetValues(14);
                    this.Log().Info($"reset values");
                    IsPositionValuesSet = false;

                    // рассчитываю значения для пробоотборника
                    ...
                    foreach (Cell cell in SelectedCells)
                    {
                        ...
                        if (cell.Offset is null)
                        {
                            this.Log().Info(
                                ...
                                $" CorrectionValue: {ConvectorHelper.CalculateCorrectionValue(rotation: cell.Rotation.Value, beakCoeff: ConnectionConfiguration?.SamplerConnection.BeakCoeff)}");
                            // расчет смещения
                    ...
                    int probesCounter = 1;
                    // временно расчет просто сортировкой 
                    var ShortestPathCells = SelectedCells.OrderBy(cell => cell.Offset).ToList();

                    //логируем координаты клетки и рассчитанные значения 
                    var cellCoordsLogMessage = $"selected sells coords:{Environment.NewLine}";
                    var cellsCoords = new List<string>();
                    
                    for (int i = 0; i < SelectedCells.Count(); i++)
                    {
                        cellsCoords.Add($"{i} px({Math.Round(SelectedCells[i].X)};{Math.Round(SelectedCells[i].Y)}); offset {SelectedCells[i].Offset}; rotation {SelectedCells[i].Rotation}");
                    }
                    cellCoordsLogMessage += string.Join(Environment.NewLine, cellsCoords);
                    this.Log().Info(cellCoordsLogMessage);

                    // записываем полученные значения в OPC сервер
                    ...
                    //await opcUaClient.WriteValue("ns=1;s=SIEMENSPLC.siemensplc.status_sps", (byte)OpcSpsStatus);
                    // Отключаем ручной режим, если он был включен
                    if (isManualSelectionOn.Value)
                    {
                    ...
                    {
                        this.Log().Error("Error while read values and log from opc", ex);
                    }

                    //saving screenshot from opc for logs
                    try
                    {
                        if (await savingSemaphore.WaitAsync(0))
                        {
                    ...
                    {
                        this.Log().Error("Error while taking screenshot and log from opc", ex);
                    }
                }
            }
            catch (Exception ex)
            {
                this.Log().Error($"Ошибка при задании значений контроллера пробоотборника: {ex} {ex.Message}");
                this.Log().Error($"Ошибка при задании значений контроллера пробоотборника: {ex} {ex.Data}");
