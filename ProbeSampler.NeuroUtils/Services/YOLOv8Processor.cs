...
namespace ProbeSampler.NeuroUtils.Services
{
    public class YOLOv8Processor : ImageProcessor, IProvideBoxes, IEnableLogger
    {
        ...
        public YOLOv8Processor(string yoloOnnxPath, double scale = 0, float confidence = 0.6f)
        {
            this.scale = scale;
            // попытка инициализации YOLO с CUDA
            try
            {
            ...
            // Иначе инициализация YOLO без CUDA
            catch (Exception ex)
            {
        ...
        public IEnumerable<BoundingBox> GetDetections(string className)
        {
            var result = detectionAccumulator.GetResult();
            if (result != null)
            {
                ...
                return result.Boxes
                    .Where(box => box.Class.Name == className)
