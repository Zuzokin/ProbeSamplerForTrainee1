...
namespace ProbeSampler.ConsoleTest
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // Выводим результат
            Console.WriteLine($"Затраченное время: {elapsedTime.TotalMilliseconds} мс");
        }
    }



    static class TestTsp
    {
        static int n = 16; // Количество вершин в графе
        ...
        static int tsp(int mask, int pos, int destination, int[,] dp, int[,] path)
        {
            if (mask == (1 << n) - 1)
                return graph[pos, destination]; // Вернуть расстояние от последней вершины к конечной
            ...
            for (int v = 0; v < n; v++)
            {
                if ((mask & (1 << v)) == 0)
                {
                    int newDistance = graph[pos, v] + tsp(mask | (1 << v), v, destination, dp, path);
        ...
        static void PrintPath(int mask, int pos, int[,] path)
        {
            if (mask == (1 << n) - 1)
                return;

            int nextVertex = path[mask, pos];
            Console.Write($"-> {nextVertex} ");
            PrintPath(mask | (1 << nextVertex), nextVertex, path);
        }
        ...
        public static void startTsp()
        {
            ...
            int minDistance = tsp(1, startVertex, destinationVertex, dp, path);
            Console.WriteLine($"Минимальное расстояние: {minDistance}");

            // Вывод оптимального пути
            Console.Write("Оптимальный путь: ");
            Console.Write($"{startVertex} ");
            PrintPath(1 | (1 << startVertex), startVertex, path);
